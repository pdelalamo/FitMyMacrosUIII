...
const TargetEnergyAndMacros: React.FC<Props> = ({ navigation }) => {
    ...
    useEffect(() => {
        const loadUserData = async () => {
            ...
            if (storedTargetCalories) setTargetCalories(storedTargetCalories);

            if (storedTargetCalories !== null) {
                if (storedTargetProtein) {
                    const proteinGrams = parseFloat(storedTargetProtein);
                    const proteinPercentage = weight === 'grams' ? (proteinGrams * 4 * 100) / parseInt(storedTargetCalories) : (proteinGrams * 4 * 100 * 28.3495) / parseInt(storedTargetCalories);
                    setProteinPercentage(Math.round(proteinPercentage));
                }
                if (storedTargetCarbs) {
                    const carbsGrams = parseFloat(storedTargetCarbs);
                    const carbsPercentage = weight === 'grams' ? (carbsGrams * 4 * 100) / parseInt(storedTargetCalories) : (carbsGrams * 4 * 100 * 28.3495) / parseInt(storedTargetCalories);
                    setCarbsPercentage(Math.round(carbsPercentage));
                }
                if (storedTargetFat) {
                    const fatGrams = parseFloat(storedTargetFat);
                    const fatPercentage = weight === 'grams' ? (fatGrams * 9 * 100) / parseInt(storedTargetCalories) : (fatGrams * 9 * 100 * 28.3495) / parseInt(storedTargetCalories);
                    setFatPercentage(Math.round(fatPercentage));
                }
            }
    ...
    const calculateMacros = () => {
        ...
        const proteinGrams = weightUnit === 'grams' ? Math.round((total * proteinPercentage) / 100 / 4) : Math.round((total * proteinPercentage) / 100 / 4 * 0.03527396195);
        const carbsGrams = weightUnit === 'grams' ? Math.round((total * carbsPercentage) / 100 / 4) : Math.round((total * carbsPercentage) / 100 / 4 * 0.03527396195);
        const fatGrams = weightUnit === 'grams' ? Math.round((total * fatPercentage) / 100 / 9) : Math.round((total * fatPercentage) / 100 / 9 * 0.03527396195);
            ...
            <ImageBackground source={require('../../assets/images/main_background.png')} resizeMode="cover" style={globalStyles.imageBackground}>
                <View style={settingsStyles.containerSettings}>
                    <ScrollView contentContainerStyle={settingsStyles.scrollViewContent}>
                        <Text style={settingsStyles.titleTarget}>{energyUnit === 'kilocalories' ? t('setupTargetCalories') : t('setupTargetKj')}</Text>
                        <TextInput
